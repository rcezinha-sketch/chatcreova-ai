...
// ===== V9: capa + Central do Projeto + Meus Prompts =====
document.getElementById("heroManager")?.addEventListener("click",()=>document.getElementById("teamAI")?.scrollIntoView({behavior:"smooth"}));
document.getElementById("heroManual")?.addEventListener("click",()=>document.getElementById("promptStudio")?.scrollIntoView({behavior:"smooth"}));
document.getElementById("heroLogin")?.addEventListener("click",()=>{
  const existing=document.querySelector('[data-account],#accountBtn,.accountBtn');
  if(existing) existing.click(); else alert("Entrar / Criar conta está preparado, mas continua EM CONSTRUÇÃO nesta versão.");
});

const V9_PROJECT_KEY="chatcreova_v9_current_project";
const pn=document.getElementById("projectNameV9"), pb=document.getElementById("projectBriefV9");
try{
 const p=JSON.parse(localStorage.getItem(V9_PROJECT_KEY)||"null");
 if(p){ if(pn)pn.value=p.name||""; if(pb)pb.value=p.brief||""; }
}catch(e){}
document.getElementById("saveProjectV9")?.addEventListener("click",()=>{
 const p={name:(pn?.value||"").trim(),brief:(pb?.value||"").trim(),updated:new Date().toISOString()};
 if(!p.name&&!p.brief){alert("Escreva o nome ou o objetivo do projeto.");return;}
 localStorage.setItem(V9_PROJECT_KEY,JSON.stringify(p));
 alert("Projeto salvo neste aparelho.");
});
document.getElementById("continueProjectV9")?.addEventListener("click",()=>{
 const task=document.getElementById("bossTask");
 const text=[pn?.value,pb?.value].filter(Boolean).join(" — ");
 if(task && text){task.value="Continue este projeto: "+text;document.getElementById("teamAI")?.scrollIntoView({behavior:"smooth"});task.focus();}
 else alert("Salve ou preencha um projeto primeiro.");
});

const V9_PROMPTS_KEY="chatcreova_v9_saved_prompts";
function renderV9Prompts(){
 const box=document.getElementById("savedPromptsV9"); if(!box)return;
 let arr=[]; try{arr=JSON.parse(localStorage.getItem(V9_PROMPTS_KEY)||"[]")}catch(e){}
 box.innerHTML=arr.length?arr.map((x,i)=>`<div class="savedPromptCard"><b>Favorito ${i+1}</b><br>${escapeHtml(x)}</div>`).join(""):"<small>Nenhum prompt salvo ainda.</small>";
}
document.getElementById("savePromptV9")?.addEventListener("click",()=>{
 const el=document.getElementById("promptSaveTextV9"), val=(el?.value||"").trim(); if(!val){alert("Cole ou escreva um prompt primeiro.");return;}
 let arr=[]; try{arr=JSON.parse(localStorage.getItem(V9_PROMPTS_KEY)||"[]")}catch(e){}
 arr.unshift(val); arr=arr.slice(0,30); localStorage.setItem(V9_PROMPTS_KEY,JSON.stringify(arr)); if(el)el.value=""; renderV9Prompts();
});
document.getElementById("clearPromptsV9")?.addEventListener("click",()=>{localStorage.removeItem(V9_PROMPTS_KEY);renderV9Prompts();});
renderV9Prompts();
