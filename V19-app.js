...
const CHANNEL_KEY = "chatcreova_v19_channels";
const PROJECT_KEY = "chatcreova_v19_projects";
const HISTORY_KEY = "chatcreova_v19_history";
...
Sua função é conversar naturalmente com o usuário, entender
o pedido e ajudar a executá-lo.

Seja útil, clara, direta e prática.

O ChatCreova possui especialistas:
...
Nunca diga que uma tarefa foi executada por uma ferramenta
externa se ela ainda não tiver realmente sido executada.

Não invente arquivos, imagens, vídeos, músicas ou resultados.
...
chaves ou informações técnicas internas.
`;
...
async function timedFetch(url, options = {}, timeout = 180000) {
...
function view(id) {
  ...
  $$(".sidebar button[data-view]").forEach(button => {
...
$("#queenVoiceToggle")?.addEventListener(
  "click",
  () => {
  ...
  cards.innerHTML =
    channels.map(channel => `
      ...
      <div class="panel">
        ...
        <p>
          ${escapeHTML(channel.platform || "")}
          ${
            channel.niche
              ? " • " + escapeHTML(channel.niche)
              : ""
          }
        </p>
        ...
        ${
          channel.language
            ? `<small>${escapeHTML(channel.language)}</small>`
            : ""
        }
...
function init() {
  ...
  /*
    Verificação interna.
    Não mostramos detalhes técnicos
    para o usuário na interface pública.
  */
